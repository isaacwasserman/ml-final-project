...
def train_simple_attention_with_tags(inf_model, inputs, tags, outputs, lang_ids=None, finetune=False, trainer=None, prev_acc=None, prev_edd=None):
    ...
    if not finetune:
        ...
        MINIBATCH_SIZE = 10
        with alive_bar(100, title=f'Burning in {L2}:') as bar:
            for i in range(100):
        ...
        prev_edd = 100
        MINIBATCH_SIZE = 1
        with alive_bar(40, title=f'Pretraining {L2}:') as bar:
            for i in range(40):
                shuffle(train_pairs)
                total_loss = 0.0
                batch = []
        ...
        trainer.restart(learning_rate)
        halvings = 0
        with alive_bar(40, title=f'Finetuning (1) {L2}:') as bar:
            for i in range(40):
        ...
        halvings = 0
        with alive_bar(40, title=f'Finetuning (2) {L2}:') as bar:
            for i in range(40):
                shuffle(indexes)
                total_loss = 0.0
                batch = []
