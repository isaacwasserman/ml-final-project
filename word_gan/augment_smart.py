...
available_epochs = [int(path.split(".")[0].split("-")[-1]) for path in glob.glob(f'checkpoints/{language}-epoch-*.h5')]

if model_epoch not in available_epochs:
    model_epoch = max(available_epochs)
...
generator.build(input_shape=[1,10,vocab_len])
generator.summary()
def hallucinate_stem(original):
	seq = "".join([lut[np.argmax(onehot)] for onehot in distribution_to_sequence(generator(tf.random.normal(shape=[1,10,vocab_len]))[0])])
	if "0" in seq:
		seq = seq[:seq.index("0")]
...
with codecs.open(os.path.join(DATA_PATH,L2+"-hall-smart"), 'w', 'utf-8') as outp:
